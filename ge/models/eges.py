...
from ..module.layers import Word2Vec, Word2VecWithSideInfo
...
class EGES:
    def __init__(
        ...
        for node in G.nodes():
            for key, val in G.nodes[node].items():
                info_dict.setdefault(key, []).append(val)
        
        info_sizes = []
        info_maps = []
        for key, vals in info_dict.items():
            unique_vals = list(set(vals))
            map_ = dict(zip(unique_vals, range(len(unique_vals))))
            
            info_dict[key] = list(map(lambda x: map_[x], vals))
            info_maps.append(map_)
            info_sizes.append(len(unique_vals))
    ...
    def train(self):
        ...
        if self.use_noise_dist == True:
            self.__init_noise_weights('degree', sentences)
        else:
        ...
        dataloader = DataLoader(dataset, batch_size=self.batch_size, shuffle=True, num_workers=8, pin_memory=True, )
        ...
        for epoch in range(self.epochs):
            ...
            for pos_u, pos_v in tqdm(dataloader):
                ...
                pos_s = self.info_vals[pos_u, :]
                
                pos_u, pos_v, neg_vs, pos_s =\
                    pos_u.to(self.device), pos_v.to(self.device), neg_vs.to(self.device), pos_s.to(self.device)
                ns_loss = self.__train(pos_u, pos_v, neg_vs, pos_s)
                epoch_loss += ns_loss
    ...
    def get_embedding(self, node):
        self.w2v_model.eval()
        with torch.no_grad():
            u_embedding = self.w2v_model.W0.weight[self.node2idx[node], :]
            side_infos = self.info_vals[self.node2idx[node], :]
            s_embeddings = [u_embedding]
            for idx, side_info in enumerate(side_infos):
                s_embeddings.append(self.w2v_model.WSs[idx].weight[side_info, :])

            s_embeddings = torch.stack(s_embeddings, dim=0)
            alpha = self.w2v_model.alpha
            embedding = torch.mul(s_embeddings,  alpha.exp()/ alpha.exp().sum()).mean(dim=0).squeeze()
            embedding = embedding.cpu().numpy()
        return embedding
    
    def get_cold_start_embedding(self, feature):
        self.w2v_model.eval()
        with torch.no_grad():
            side_infos = [self.info_maps[i][feat_idx] for i, feat_idx in enumerate(feature)]
            s_embeddings = []
            for idx, side_info in enumerate(side_infos):
                s_embeddings.append(self.w2v_model.WSs[idx].weight[side_info, :])

            s_embeddings = torch.stack(s_embeddings, dim=0)
            alpha = self.w2v_model.alpha[1:, ]
            embedding = torch.mul(s_embeddings,  alpha.exp()/ alpha.exp().sum()).mean(dim=0).squeeze()
            embedding = embedding.cpu().numpy()
        return embedding
    ...
    def __train(self, pos_u, pos_v, neg_vs, pos_s):
        ns_loss = self.w2v_model(pos_u, pos_v, neg_vs, pos_s)
